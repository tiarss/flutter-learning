    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        if (constraints.maxWidth > 800) {
          return DetailsScreenDesktop(place: place);
        } else {
          return DetailsScreenMobile(place: place);
        }
      },
    );
  }
...
class DetailsScreenMobile extends StatelessWidget {
  final TourismPlace place;
  const DetailsScreenMobile({Key? key, required this.place}) : super(key: key);
...
class DetailsScreenDesktop extends StatefulWidget {
  ...
  const DetailsScreenDesktop({Key? key, required this.place}) : super(key: key);
